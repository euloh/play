...
`root`, you must change the mode of the DTrace
...
Alternatively, if the `acl` package is
...
You can also create a `udev` rules file such as
`/etc/udev/rules.d/10-dtrace.rules` to change
...
following to the `udev` rules file:

```
kernel=="dtrace/helper", RUN="/usr/bin/setfacl -m u:guest:rw /dev/dtrace/helper"
```

To apply the `udev` rule without needing to
...
opportunity rather than completeness or efficiency.
        ...
        *only* and is not intended to efficiently
...
`makefile` file.
          ...
          that the `make` command can function
...
Running the `make` command creates an
executable named ``prime``, which can be run to
...
`prime` executable, you can practice adding
USDT probes to an application, as described in
[Adding USDT Probes to an Application](#adding_usdt_probes_to_an_application).
...
To get started, you will need to create a `.d`
...
[Oracle Linux: DTrace Reference Guide](../doc/userguide/index.md).
        ...
        This `.d` file is not a script that is run in
        the same way that is shown in previous examples in this
        tutorial, but is rather the `.d` source file
...
After creating the `.d` file, you then need to
create the required probe points to use in the following examples.
This information is added to the `primain.c`
...
completed and checked.
                     ...
                     This table includes a description of the data and points to be probed after user entry is completed and checked.
                     Each probe point represents a sequence of operations.

                     | Description                              | Probe                       |
                     | User entry complete and checked          | `userentry( int )`          |
                     | Return and input to `seekFactorA()`      | `factorreturnA( int, int )` |
                     | Return and input to `seekFactorB()`      | `factorreturnB( int, int )` |
...
The next step is to create a `.h` file from the
`dprime.d` file, as shown here:

```
# dtrace -h -s dprime.d
```

The `dprime.h` file that is created contains a
reference to each of the probe points that are defined in the
`dprime.d` file.

Next, in the application source file,
`primain.c`, we add a reference to the
`#include "dprime.h"` file and add the
appropriate probe macros at the proper locations.

In the resulting `primain.c` file, the probe
...
as the macros listed in the `dprime.h` file.
...
Add a target that instructs `dtrace` to
re-create the `dprime.h` file in the event that
changes are subsequently made to the `dprime.d`
file. This step ensures that you do not have to manually run the
`dtrace -h -s dprime.d` command if any changes
are made.

This exercise also has you direct `dtrace` to
create a `prime.o` file.
...
```
        ...
        `provider primeget`, from the
        `dprime.d` file, and the PID of the running
        application `prime`. The output of the
...
`root`, the helper device must have the proper
permissions. Alternatively, you can run the program with the
probes in it as the `root` user. See
[Example: Changing the Mode of the DTrace Helper Device](#example_changing_the_mode_of_the_dtrace_helper_device)
for more information about changing the mode of the DTrace helper device.
...
`root` user can send probe provider information
to DTrace:
