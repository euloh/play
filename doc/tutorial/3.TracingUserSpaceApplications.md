...
More accurately, there is a different such provider for each process,
meaning that we append the process ID to specify the process we want to trace.
For example, we can run `pgrep prime` to get the process ID.
In our case, it is 2889188.
We can list the `pid` probes associated with this process in either of two ways:
